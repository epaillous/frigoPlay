/**
 * cvi_reflex_lib.js 1.5 (10-Aug-2010) (c) by Christian Effenberger 
		...
		if(image.tagName.toUpperCase() == "IMG") {
			...
			try {
				...
				if(document.all&&document.namespaces&&!window.opera&&(!document.documentMode||document.documentMode<9)) {
					...
					var temp,canvas = document.createElement('canvas');
					if(canvas.getContext("2d")) {
						canvas.options = options;
						temp=navigator.userAgent.indexOf('WebKit')!=-1?true:false;
						canvas.isWK=(temp==true?window.postMessage?false:window.external?false:true:false);
						canvas.isCR=(temp==true&&window.external?true:false);
						canvas.bm = m;
						canvas.id = image.id;
						canvas.alt = image.alt;
						canvas.name = image.name;
					...
					img.onload = function() {
						...
						if(typeof resource=='undefined') {
							...
							if(resource.getContext) {
								...
								if(canvas.isWK) {obj.appendChild(resource);}
							}
						}
						...
						if(!canvas.isWK||itilt=='n') {
							...
							styl = context.createLinearGradient(0,boxh-iheit,0,boxh);
							styl.addColorStop(1,"rgba(0,0,0,1.0)");
							styl.addColorStop(0,"rgba(0,0,0,"+iopac+")"); 
							context.fillStyle = styl;
						}
						if(canvas.isWK) {
							context.beginPath();
							...
							if(resource.getContext) {
								...
								if(canvas.getContext) {
									...
									if(canvas.isWK) {
										context.save();
										context.globalCompositeOperation="destination-out";
										styl = context.createLinearGradient((itilt=='l'?ww:0),boxh-iheit,(itilt=='l'?ww-(2*(wide/divs)):2*(wide/divs)),boxh);
										styl.addColorStop(1,"rgba(0,0,0,1.0)");
										styl.addColorStop(0,"rgba(0,0,0,"+iopac+")");
										context.fillStyle = styl;
